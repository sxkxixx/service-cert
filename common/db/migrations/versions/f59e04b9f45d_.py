...
Revision ID: f59e04b9f45d
Revises: 
Create Date: 2025-01-08 19:57:05.496143
...
def upgrade() -> None:
    ...
    sa.Column('status', postgresql.ENUM('NEW', 'GENERATING_CONFLUENCE_SPACE', 'NEED_CREATE_RELEASE_FOLDER', 'CREATING_RELEASE_FOLDER', 'NEED_UPDATE_HOMEPAGE', 'READY', name='servicestatus'), nullable=False),
    ...
    op.create_table('service_requirement',
    sa.Column('service_id', sa.Uuid(), nullable=False),
    ...
    sa.ForeignKeyConstraint(['service_space_id'], ['service_space.id'], ),
    sa.PrimaryKeyConstraint('id')
    )
    op.create_table('release_requirement',
    sa.Column('release_id', sa.Uuid(), nullable=False),
    sa.Column('responsible_id', sa.Uuid(), nullable=True),
    sa.Column('id', sa.Uuid(), nullable=False),
    sa.Column('created_at', sa.TIMESTAMP(timezone=True), nullable=False),
