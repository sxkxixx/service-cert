...
class TimestampMixin:
    ...
    def created_at(self) -> Mapped[datetime]:
        if hasattr(self, "_sa_apply_dc_transforms"):  # проверка, является ли наследником MappedAsDataclass
    ...
    def updated_at(self) -> Mapped[datetime]:
        if hasattr(self, "_sa_apply_dc_transforms"):  # проверка, является ли наследником MappedAsDataclass
            return mapped_column(
                TIMESTAMP(timezone=True), server_onupdate=func.now(), server_default=func.now(), init=False
            )
        return mapped_column(TIMESTAMP(timezone=True), server_onupdate=func.now(), server_default=func.now())
